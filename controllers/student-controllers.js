...
export const studentregistraion=async(request,response)=>{
    ...
    try {
        ...
        const {email,name,dob,age,gender,address,parentname,standard,watsappcontact,normalcontact,profileImage}=request.body;
...
export const enrollStudentInCourse = async(request,response)=>{
    ...
    try {
        ...
        const isEnrolled = student.courseEnrollments.some(item => item.courseId == courseId);

        if (isEnrolled) {
            return response.status(401).json({ message: 'Student is already registered in this course',status:401 });
        }

        const updatedStudent = await Student.findByIdAndUpdate(studentId, {
            $push: {
                courseEnrollments: { courseId, feesPaid,feesAmountPaid,Balance,DateOfEnrollment,fees}
            }
        }, { new: true });

        if (!updatedStudent) {
            return response.status(404).json({ message: "failed to update",status:"404" });
        }
        ...
        sendEmail(emaill, subject, text);
            ...
            return response.status(200).json({ message: 'Student enrollment updated', student: updatedStudent,status:200 });
...
export const getStudentCourses = async (request, response) => {
    ...
    try {
        const student = await Student.findById(studentId).populate('courseEnrollments.courseId');
        if (!student) {
            return response.status(404).json({ message: 'Student not found' });
        }
        const courses = student.courseEnrollments.map(enrollment => enrollment.courseId);
        const filtercoursespaid=student.courseEnrollments.filter(enrollment => enrollment.feesPaid == true);
        const filtercoursesnotpaid=student.courseEnrollments.filter(enrollment => enrollment.feesPaid == !true);
        if(paid !== undefined){
            if(paid == "true"){
...
export const updateFeesPaidStatus = async (request, response) => {
    const { studentId, courseId, AmountPaid } = request.params;
    console.log("amtttt",AmountPaid);
    // const bal=3000;
    const student=await Student.findById({_id:studentId})
    console.log("studenttttt",student);
    const course = student.courseEnrollments.filter(item => item.courseId == courseId);
    console.log("courseeeee",course);
    const Balance=course[0].Balance-AmountPaid;
    ...
    try {
        const updatedStudent = await Student.findOneAndUpdate(
            ...
            { $set: { 'courseEnrollments.$.feesAmountPaid': AmountPaid,'courseEnrollments.$.Balance': Balance } },
            { new: true } // Return the updated document
        );

        if (!updatedStudent) {
            return response.status(404).json({ message: 'Student or course not found',status:404 });
        }

        return response.status(200).json({ message: 'FeesPaid status updated successfully', student: updatedStudent,status:200 });
    } catch (error) {
...
export const addStudentAttendance=async(req,res)=>{
    ...
    try {
        ...
        if (!updatedStudent) {
            return res.status(404).json({ message: 'Student or course not found',status:404 });
        }

        return res.status(200).json({ message: 'Attendance updated successfully', student: updatedStudent,status:200 });
    } catch (error) {
        console.error('Error updating feesPaid status:', error);
...
export const getStudentByCourse=async(req,res)=>{
    ...
    try {
            ...
            return res.status(200).json({ message: 'fetched data sucessfully', student: getStudent,status:200 });
    } catch (error) {
        return res.status(500).json({ message: 'Internal server error' });
    }
}
...
export const studentById=async(req,res)=>{
    ...
    try {
        const studentData = await Student.findById(id).populate('courseEnrollments.courseId');
        // console.log('====================================');
